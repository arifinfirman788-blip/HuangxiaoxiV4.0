...
import { 
  ...
  Menu, ScanLine, Phone, Star, Ticket, Map
} from 'lucide-react';
import ScenicAvatar from '../../image/张手跑_1.png'; 
...
const ScenicAgent = () => {
  ...
  // Mock Data
  const staffList = [
    { name: '王导', role: '金牌讲解', desc: '听故事找我', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=King' },
    { name: '小张', role: '安全员', desc: '求助找我', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=John' },
    { name: '李姐', role: '票务', desc: '购票找我', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Lisa' },
    { name: '小陈', role: '观光车', desc: '乘车找我', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Chen' },
  ];

  const quickActions = [
    { icon: Map, text: '景区电子导览图' },
    { icon: Calendar, text: '演出时间表' },
    { icon: Bus, text: '观光车路线查询' },
  ];
  ...
  return (
    <div className="h-full w-full bg-slate-50 relative flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pb-[180px]">
        ...
        <div className="absolute top-0 left-0 right-0 h-[400px] bg-gradient-to-b from-emerald-100 via-emerald-50/50 to-transparent z-0 pointer-events-none" />
        ...
        <div className="relative z-10 px-6 mb-6">
          <div className="flex justify-between items-start">
             <div className="w-28 h-28 relative -ml-2">
                <img src={ScenicAvatar} alt="Avatar" className="w-full h-full object-contain drop-shadow-xl transform hover:scale-105 transition-transform" />
             </div>
             <div className="bg-white/60 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-1 text-[10px] text-slate-600 shadow-sm border border-white/50">
                <MapPin size={10} />
                <span>安顺市 18°C 晴</span>
             </div>
          </div>
          
          <div className="mt-[-40px] ml-[110px] flex items-center gap-2">
             <h1 className="text-2xl font-bold text-slate-800 tracking-tight whitespace-nowrap">欢迎来到，黄果树瀑布</h1>
          </div>
        </div>

        {/* Main Service Card */}
        <div className="px-4 relative z-10">
          <div className="bg-white/40 rounded-[1.5rem] p-5 shadow-xl shadow-emerald-100/50 border border-white/50 backdrop-blur-xl">
             <div className="flex items-start gap-2 mb-4">
                <div className="w-4 h-4 mt-1 text-emerald-500">
                  <Star size={16} fill="currentColor" />
                </div>
                <p className="text-sm text-slate-600 leading-relaxed font-medium">
                  我是您的景区向导，可以为您提供路线规划、景点讲解及票务服务
                </p>
             </div>
             ...
             <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between border border-slate-100">
                <div className="flex items-center gap-2 flex-1">
                   <span className="text-sm font-bold text-slate-700 whitespace-nowrap">票号/码：</span>
                   <input 
                     type="text" 
                     className="bg-transparent outline-none w-full text-sm font-medium text-slate-800 placeholder-slate-400"
                   />
                </div>
                <div className="flex items-center gap-1 text-xs text-emerald-500 font-medium cursor-pointer hover:opacity-80">
                   <span>扫描门票码获取导览</span>
                   <ScanLine size={14} />
                </div>
             </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 z-20 pointer-events-none">
         ...
         <div className="flex items-center gap-2 mb-3 overflow-x-auto scrollbar-hide pointer-events-auto">
            ...
            <div className="ml-auto">
               <button className="px-4 py-1.5 bg-white rounded-full text-xs font-bold text-emerald-500 shadow-sm border border-emerald-100 flex items-center gap-1 whitespace-nowrap">
         ...
         <div className="bg-white rounded-full p-2 pl-4 flex items-center gap-3 shadow-lg border border-slate-100 pointer-events-auto">
            ...
            <input 
              ...
              placeholder="请输入您需要的景区服务"
              className="flex-1 bg-transparent outline-none text-sm text-slate-800 placeholder-slate-400"
            />
            <button className="w-9 h-9 bg-emerald-500 rounded-full flex items-center justify-center text-white shadow-md hover:bg-emerald-600 transition-colors">
